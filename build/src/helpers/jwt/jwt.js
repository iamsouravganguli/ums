...
import { access_private_Key, access_public_Key, refresh_private_Key, refresh_public_Key, } from '../../rsa/rsa.js';
import { User } from '../../database/models/user.model.js';
export const JWT = {
    access_token: async (payload) => {
        try {
    ...
    verify_access_token: async (req) => {
        try {
            const jwtString = req.headers.authorization && req.headers.authorization.split(' ')[1];
            if (!jwtString) {
    ...
    verify_refresh_token: async (req) => {
        try {
            ...
            if (!jwtString) {
                ...
                const payload = jsonwebtoken.verify(jwtString, refresh_public_Key);
